...
# If running against remote eval and no explicit KB path was provided, prefer the
# common Brev path so remote workers resolve local dataset mode correctly.
if [ -n "$REMOTE_EVAL_URL" ] && [ -z "${KERNELSWARM_KB_REPO_PATH:-}" ] && [ ! -d "$KB_REPO_PATH" ]; then
